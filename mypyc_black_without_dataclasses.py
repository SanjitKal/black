...
@click.option(
    ...
    # MYPYC: mypy cannot infer the type of v correctly, so mypyc attempts a coercion
    # which results in a runtime error. The commented out lambda is replaced
    # with the above function called "click_option_callback" with
    # appropriately annotated arguments.
    # callback=lambda c, p, v: [TargetVersion[val.upper()] for val in v],
    callback=click_option_callback,
    multiple=True,
    help=(
...
def main(
    ...
    include: str,
    exclude: str,
    # MYPYC: gave src the correct type (used to be Tuple[str], but src was an
    # empty tuple in one or more cases)
    src: Tuple[str, ...],
    config: Optional[str],
) -> None:
...
def format_file_in_place(
    ...
    """
    if src.suffix == ".pyi":
        mode = FileMode(mode.target_versions, mode.line_length, mode.string_normalization, True)
    ...
    # MYPYC: The if and elif predicates had to be changed from the commented out
    # code to the new predicates because mypy infers the type of write_back,
    # which by default is equal to WriteBack.NO, as type
    # Enum, but it infers the type of write_back.YES to be of type int.
    # According to Ivan, this is an unintended result of the typechecker,
    # but he is fine prohibiting this sort of enum attribute access, so
    # the predicates below were rewritten (otherwise we get a runtime error
    # with int object expected.)
...
class DebugVisitor(Visitor[T]):
    ...
    def __init__(self, tree_depth: int = 0) -> None:
...
class BracketTracker:
    ...
    def __init__(
        self,
        depth: int = 0,
        bracket_match: Optional[Dict[Tuple[Depth, NodeType], Leaf]] = None,
        delimiters: Optional[Dict[LeafID, Priority]] = None,
        previous: Optional[Leaf] = None,
        _for_loop_depths: Optional[List[int]] = None,
        _lambda_argument_depths: Optional[List[int]] = None,
    ) -> None:
        self.depth = depth
        self.bracket_match = bracket_match if bracket_match is not None else {}
        self.delimiters = delimiters if delimiters is not None else {}
        self.previous = previous
        self._for_loop_depths = _for_loop_depths if _for_loop_depths is not None else []
        self._lambda_argument_depths = _lambda_argument_depths if _lambda_argument_depths is not None else []
...
class Line:
    ...
    def __init__(
        ...
        comments: Optional[Dict[LeafID, List[Leaf]]] = None,
        bracket_tracker: Optional[BracketTracker] = None,
        inside_brackets: bool = False,
        should_explode: bool = False,
    ) -> None:
        self.depth = depth
        ...
        self.bracket_tracker = bracket_tracker if bracket_tracker is not None else BracketTracker()
...
class EmptyLineTracker:
    ...
    def __init__(
        ...
        previous_line: Optional[Line] = None,
        previous_after: int = 0,
        previous_defs: Optional[List[int]] = None,
    ) -> None:
        self.is_pyi = is_pyi
...
def left_hand_split(line: Line, features: Collection[Feature] = ()) -> Iterator[Line]:
    ...
    # MYPYC: This annotations needed to be added otherwise mypy cannot infer that the
    # type of matching_bracket is really Optional[Leaf].
    # This mistype results in mypyc throwing a ("None object expected") runtime error
    # due to the 'leaf.opening_bracket is matching_bracket' check below.
    matching_bracket = None  # type: Optional[Leaf]
...
def right_hand_split(
    ...
    # MYPYC: These annotations need to be added otherwise mypy cannot infer that the
    # type of opening_bracket and closing_bracket is really Optional[Leaf], not None.
    # This mistype results in mypyc throwing a ("None object expected") runtime error
    # due to the 'leaf is opening_bracket' check below.
    opening_bracket = None  # type: Optional[Leaf]
    closing_bracket = None  # type: Optional[Leaf]
    ...
    for leaf in node.leaves():
        ...
        for comment in list_comments(leaf.prefix, is_endmarker=False):
            if comment.value in FMT_OFF:
                ...
                # MYPYC: This annotation needed to be added otherwise mypy cannot infer that the
                # type of first_idx is really Optional[int].
                # This mistype results in mypyc throwing a ("None object expected") runtime error
                # due to the 'if first_idx is None' check below.
                first_idx = None  # type: Optional[int]
...
def unwrap_singleton_parenthesis(node: LN) -> Optional[LN]:
    ...
    # MYPYC: The Leaf class does not have attribute "children",
    # but type of node is LN (which is Union[Leaf, Children]).
    # statement causes a runtime error saying "list object expected".
    # As a result, the below isinstance check was inserted
    if isinstance(node, Leaf):
        return None
...
def generate_trailers_to_omit(line: Line, line_length: int) -> Iterator[Set[LeafID]]:
    ...
    length = 4 * line.depth
    # MYPYC: These annotations needed to be added otherwise mypy cannot infer that the
    # type of opening_bracket is really Optional[Leaf].
    # This mistype results in mypyc throwing a ("None object expected") runtime error
    # due to the 'if opening_bracket' check below.
    opening_bracket = None  # type: Optional[Leaf]
    closing_bracket = None  # type: Optional[Leaf]
